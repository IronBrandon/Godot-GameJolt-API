...
var _username_cache: String ## The currently cached username.
var _token_cache: String ## The currently cached token.
...
func _call_gj_api(type: String, parameters: Dictionary = {}, sub_types: Array = []) -> void:
	var request_error: Error = OK
	parameters["username"] = parameters.get("username", _username_cache)
	parameters["user_token"] = parameters.get("user_token", _token_cache)
	if busy:
...
func get_token() -> String: return _token_cache ## Returns the user token cache.
...
func user_auto_auth() -> void:
	...
	if tmp is String:
		...
		if tmp is String:
			_token_cache = tmp
			_call_gj_api('/users/auth/', {user_token = _token_cache, username = _username_cache})

## Attempts to authenticate a user with the given [param username] and [param token]. [br][br]Request type is "/users/auth/".
func user_auth(username: String, token: String) -> void:
	_call_gj_api('/users/auth/',
		{user_token = token, username = username})
	_username_cache = username; _token_cache = token
...
func scores_fetch_global(limit=null, table_id=null, better_than=null, worse_than=null):
	_call_gj_api('/scores/',
		{username = null, user_token = null, limit = limit, table_id = table_id, better_than = better_than, worse_than = worse_than})

## Attempts to add a score to the cached user or guest.
func scores_add(score, sort, guest: String = "", table_id=null):
	var parameters = {score = score, sort = sort, table_id = table_id}
	if _username_cache && _token_cache:
		parameters["username"] = _username_cache; parameters["user_token"] = _token_cache
	else:
		parameters["guest"] = guest
	# Attempt call
	if ((parameters["username"] && parameters["user_token"])\
		|| parameters["guest"]):
			_call_gj_api('/scores/add/', parameters)
...
## Unlocks a new trophy for the authenticated user. [i]User must be authenticated[/i]
func trophy_add_achieved(trophy_id) -> void:
	if _username_cache != null: _call_gj_api('/trophies/add-achieved/',
		{trophy_id = trophy_id})

## Removes a trophy from the authenticated user. [i]User must be authenticated[/i]
